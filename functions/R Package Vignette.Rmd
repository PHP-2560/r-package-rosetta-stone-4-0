...
```{r, eval=FALSE}
lgraph_key(climate_table, 6)
```

![](C:/Users/nmerc/Desktop/sample_line graph_r.png)
...
Here are a few helpful tips for using the 'lgraph_key()' function: 

* Similar to the 'bgraph_key()' function, the user must run the function 'key_data()' first and save the resulting data frame prior to using 'lgraph_key()'. 

* Unlike the 'bgraph_key()' function, there is no recommended limit to the number of key terms a user should plot, however, the line graphs may become clustered and diffucult to read if the user has a significant number of years in the search or plots a significant number of keywords.
